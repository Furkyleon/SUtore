...
const ProfilePage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/update_user_fields/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setMessage(errorData.error || 'Failed to update profile. Please try again.');
      }
    } catch (error) {
  ...
  return (
    <div className="profile-page-container">
      ...
      <form onSubmit={handleSubmit} className="profile-page-form">
        ...
        <div className="profile-page-form-group">
          <label htmlFor="taxId" className="profile-page-label">Tax ID:</label>
          <input
            type="text"
            id="taxId"
            value={taxId}
